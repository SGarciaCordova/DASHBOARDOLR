...
import io
import pandas as pd
from src import data_loader, kpi_engine, alert_engine, ml_predictor
...
with st.sidebar:
    ...
    st.download_button(
        ...
        data=buffer,
        file_name=f"Data_Dashboard_{date.today()}.xlsx",
...
<div class="main-scroll-area">
    ...
    <div class="grid-6">
        ...
        <div class="card" onclick="showModal('cumpl_72h')">
            <div class="card-header"><div class="card-label">SLA Compliance 72h<span class="card-sub">Cumplimiento de SLA</span></div><div class="card-icon" style="background:#d1fae5;">⏰</div></div>
            <div class="card-value" style="color:var(--green);">{cumpl_72h['pct']:.1f}%<span class="wow-change {'wow-up' if wow_sla >= 0 else 'wow-down'}" title="{wow_label}">{'▲' if wow_sla >= 0 else '▼'}{abs(wow_sla):.1f}%</span></div>
            <div class="card-change">{cumpl_72h['cumple']}/{cumpl_72h['total']} on time</div>
        </div>

        <div class="card" onclick="showModal('tiempo_ing')">
            <div class="card-header"><div class="card-label">Dock to Stock Time<span class="card-sub">Tiempo de Ingreso</span></div><div class="card-icon" style="background:#fef3c7;">⏱️</div></div>
            <div class="card-value" style="color:var(--orange);">{tiempo_ing['promedio']:.1f}d</div>
            <div class="card-change">Avg Days (Promedio)</div>
        </div>
        <div class="card" onclick="showModal('vol_recib')">
            <div class="card-header"><div class="card-label">Inbound Volume<span class="card-sub">Volumen de Entrada</span></div><div class="card-icon" style="background:#e0f2fe;">📦</div></div>
            <div class="card-value" style="color:var(--purple);">{vol_recib['piezas']:,.0f}</div>
            <div class="card-change">{vol_recib['cajas']:,.0f} boxes (cajas)</div>
        </div>
        <div class="card" onclick="showModal('tiempo_extra')">
            <div class="card-header"><div class="card-label">SLA Breach (Overtime)<span class="card-sub">Tiempo Excedido</span></div><div class="card-icon" style="background:#fee2e2;">⚠️</div></div>
            <div class="card-value" style="color:var(--red);">{tiempo_extra['excedidos']}</div>
            <div class="card-change">{tiempo_extra['pct_excedido']:.1f}% breach rate</div>
        </div>
        <div class="card" onclick="showModal('efic_desc')">
            <div class="card-header"><div class="card-label">Unloading Efficiency<span class="card-sub">Eficiencia Descarga</span></div><div class="card-icon" style="background:#ccfbf1;">🚛</div></div>
            <div class="card-value" style="color:var(--cyan);">{efic_desc['eficiencia']:.1f}%</div>
            <div class="card-change">{efic_desc['en_meta']} within target</div>
        </div>
        <div class="card" onclick="showModal('risk_prediction')">
            ...
            <div class="card-value" style="color:#be185d;">{risk_prediction.get('high_risk_count', 0)}</div>
    ...
    <div class="grid-6">
        <div class="card" onclick="showModal('cumpl_entrega')">
            <div class="card-header"><div class="card-label">OTIF (On Time In Full)<span class="card-sub">Entregas Perfectas</span></div><div class="card-icon" style="background:#fef3c7;">🎯</div></div>
            <div class="card-value" style="color:var(--orange);">{cumpl_entrega['pct']:.1f}%<span class="wow-change {'wow-up' if wow_otif >= 0 else 'wow-down'}" title="{wow_label}">{'▲' if wow_otif >= 0 else '▼'}{abs(wow_otif):.1f}%</span></div>
            <div class="card-change">{cumpl_entrega['on_time']}/{cumpl_entrega['total']} on time</div>
        </div>
        <div class="card" onclick="showModal('pct_surtido')">
            <div class="card-header"><div class="card-label">Fill Rate<span class="card-sub">Tasa de Surtido</span></div><div class="card-icon" style="background:#d1fae5;">📊</div></div>
            <div class="card-value" style="color:var(--green);">{pct_surtido['pct']:.1f}%</div>
            <div class="card-change">{pct_surtido.get('surtido', 0):,.0f}/{pct_surtido.get('total', 0):,.0f} piezas</div>
        </div>
        <div class="card" onclick="showModal('audit_quality')">
            <div class="card-header"><div class="card-label">Fulfillment Completion<span class="card-sub">Cumplimiento Surtidos</span></div><div class="card-icon" style="background:#ccfbf1;">📊</div></div>
            <div class="card-value" style="color:var(--cyan);">{audit_quality['pct']:.1f}%<span class="wow-change {'wow-up' if wow_fulfillment >= 0 else 'wow-down'}" title="{wow_label}">{'▲' if wow_fulfillment >= 0 else '▼'}{abs(wow_fulfillment):.1f}%</span></div>
            <div class="card-change">{audit_quality['total']} surtidos procesados</div>
        </div>
        <div class="card" onclick="showModal('vol_surtido')">
            <div class="card-header"><div class="card-label">Outbound Throughput<span class="card-sub">Volumen de Salida</span></div><div class="card-icon" style="background:#ede9fe;">📦</div></div>
            <div class="card-value" style="color:var(--purple);">{vol_surtido['surtido']:,.0f}</div>
            <div class="card-change">{vol_surtido['ordenes']} orders</div>
        </div>
        <div class="card" onclick="showModal('avance_etapa')">
            <div class="card-header"><div class="card-label">Pipeline Velocity<span class="card-sub">Velocidad de Flujo</span></div><div class="card-icon" style="background:#eff6ff;">📊</div></div>
            <div class="card-value" style="color:var(--blue);">{avance_etapa['surtido']:.0f}%</div>
            <div class="card-change">Global Picking Avg</div>
        </div>
        <div class="card" onclick="showModal('wip_metrics')">
            <div class="card-header"><div class="card-label">Active Work (WIP)<span class="card-sub">Avance Ordenes Activas</span></div><div class="card-icon" style="background:#e0f2fe;">🚧</div></div>
            <div class="card-value" style="color:var(--blue);">{wip_metrics['avance']:.1f}%</div>
            <div class="card-change">{wip_metrics['piezas_pendientes']:,.0f} pzas pendientes</div>
        </div>
        <div class="card" onclick="showModal('backlog')">
            <div class="card-header"><div class="card-label">Order Backlog<span class="card-sub">Pedidos Pendientes</span></div><div class="card-icon" style="background:#fee2e2;">📋</div></div>
            <div class="card-value" style="color:var(--red);">{backlog['display_backlog']}</div>
