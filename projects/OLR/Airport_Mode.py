...
# PERSISTIR NAVEGACIÓN
st.query_params["page"] = "flight_on"

# MANDATORY SECURITY CHECK: Protect the module from direct access
...
if not df_surtidos.empty:
    ...
    # We now prioritize the 'dt_entregado' automatically generated by kpi engine, then fallbacks
    comp_date_cols = ['dt_entregado', 'FECHA / HORA ENTREGADO', 'FECHA ENTREGADO', 'dt_promesa', 'FECHA A ENTREGAR', 'FECHA']
    
    for col in comp_date_cols:
        if col in df_completed.columns:
            # Clean string briefly to avoid parse errors if it's not datetime already
            clean_dates = df_completed[col].astype(str).str.replace(r'\s+', '', regex=True)
            df_completed['fecha_fin'] = pd.to_datetime(clean_dates, dayfirst=False, errors='coerce')
            
            # If the column yielded at least one valid date, we use it
            if df_completed['fecha_fin'].notna().any():
                break
    else:
        df_completed['fecha_fin'] = pd.NaT # Fallback

    # Any remaining NaT gets pushed to bottom of sort, so let's fill NaT with a very old date just to be safe,
    # or actually we WANT them sorted by some date.
    df_completed['fecha_fin'] = df_completed['fecha_fin'].fillna(pd.Timestamp("2000-01-01"))
    
    # Sort completed: Increase limit to 100 to reveal older rows like March 31st
    df_completed = df_completed.sort_values('fecha_fin', ascending=False).head(100)
    ...
    for _, row in df_active.head(25).iterrows(): # Slightly more items
        ...
        if pd.notna(fecha_prom):
            if fecha_prom.hour == 23 and fecha_prom.minute == 59 and fecha_prom.second == 59:
                prom_str = fecha_prom.strftime('%d/%m/%Y') + " Fin día"
            else:
                prom_str = fecha_prom.strftime('%d/%m/%Y %H:%M')
        else:
            prom_str = '-'
    ...
    for _, row in df_completed.iterrows():
        ...
        fin_str = fin.strftime('%d/%m/%Y %H:%M') if pd.notna(fin) else 'Sin Fecha'
